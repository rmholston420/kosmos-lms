"""Stage 8.5 — kernel-boot Tektos executor/tool-router wiring (ADR-107 D6).

Fast-tier tests over the two ``KOSMOS_TEKTOS_{EXECUTOR,TOOL_ROUTER}`` env-gates
wired into ``kernel/app.py``. Scenarios per ADR-107 D6:
...
- Boot order       → two slots wire AFTER _boot_tektos_decomposer.
- TektosPlugin dataclass fields exist for executor + tool_router (ADR-107 D5).
- _BootRegistry has both new slots.
"""
...
for _v in (
    "KOSMOS_TEKTOS_EXECUTOR",
    "KOSMOS_TEKTOS_TOOL_ROUTER",
):
...
_SLOTS = ("executor", "tool_router")
_ENVS = {
    "executor": "KOSMOS_TEKTOS_EXECUTOR",
    "tool_router": "KOSMOS_TEKTOS_TOOL_ROUTER",
}
...
def test_on_without_relational_memory_degrades_to_none(
    ...
    """ADR-107 D6: ``on`` requires ``registry.relational_memory`` non-None.
...
# ---------------------------------------------------------------------------
# Boot order: two new slots wire AFTER tektos_decomposer (Stage 8.4)
# ---------------------------------------------------------------------------


@pytest.mark.parametrize("slot", _SLOTS)
def test_boot_order_engine_slot_after_decomposer(slot: str) -> None:
    """Static ordering guarantee: source-level position of each
    ``registry.tektos_<slot> = _boot_tektos_<slot>`` assignment must be
    AFTER the ``registry.tektos_decomposer = _boot_tektos_decomposer``
    assignment (Stage 8.4 baseline)."""
    import inspect

    from kernel import app as kernel_app_module

    src = inspect.getsource(kernel_app_module)
    idx_dec = src.index("registry.tektos_decomposer = _boot_tektos_decomposer")
    idx_slot = src.index(f"registry.tektos_{slot} = _boot_tektos_{slot}")
    assert idx_dec < idx_slot, (
        f"ADR-107 D6 requires _boot_tektos_{slot} to run AFTER "
        f"_boot_tektos_decomposer so registry.relational_memory has "
        f"already been booted."
    )


# ---------------------------------------------------------------------------
# _BootRegistry slots exist for both engines
# ---------------------------------------------------------------------------


@pytest.mark.parametrize("slot", _SLOTS)
def test_boot_registry_has_stage_8_5_engine_slot(slot: str) -> None:
    from kernel import app as kernel_app_module

    assert hasattr(kernel_app_module.registry, f"tektos_{slot}"), (
        f"ADR-107 D6 requires _BootRegistry to expose tektos_{slot}"
    )


# ---------------------------------------------------------------------------
# TektosPlugin dataclass field surface (ADR-107 D5)
# ---------------------------------------------------------------------------


def test_tektos_plugin_dataclass_carries_stage_8_5_engine_fields() -> None:
    """ADR-107 D5 amends the ``TektosPlugin`` dataclass with two new
    optional fields — ``executor``, ``tool_router`` — each defaulting to
    ``None``."""
    import dataclasses
    ...
    for slot in _SLOTS:
        assert slot in field_names, f"TektosPlugin missing ADR-107 D5 field: {slot}"
