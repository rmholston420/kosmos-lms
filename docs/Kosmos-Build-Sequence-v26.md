...
### Stage 5.6 — Self-improvement + self-repair (gated)

- **Ports touched:** `SelfModificationPort` (**ADR-090 — PROPOSED; DEFERRED**), `ApprovalPort`, `MemoryPort`, `EventBusPort`
- **What lands:** self-improvement (`plugins/tektos/self_improve/`) and self-repair (`plugins/tektos/self_repair/`) sub-packages ported from Tektos-Ultima with all self-modifying paths gated behind `ApprovalPort`; **no** filesystem mutation until ADR-090 ratifies the formal port.
- **DoD:** every self-modification write to `MemoryPort` carries `provenance=tektos_self_modification` with `confidence ≤ 0.9`; a scripted self-modification proposal round-trips through `ApprovalPort` and is denied without user approval; ADR-090 remains PROPOSED (no ratification in this stage).
