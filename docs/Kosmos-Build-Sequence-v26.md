...
- **Ports touched:** `MemoryPort`, `MemoryPort.search_hybrid` (ADR-085)
- **What lands:** hindsight data migrated into DozerDB via a one-shot migration script writing through the DozerDB `MemoryPort` adapter with `provenance=hindsight_bridge` preserved; `adapters/memory/hindsight_bridge/` retired; all Tektos reads/writes switch to `MemoryPort.search_hybrid` directly; hindsight port `:9000` released.
- **DoD:** migration script idempotent (re-run leaves store unchanged); Tektos smoke-test suite green with `hindsight_bridge` absent from `pyproject.toml`; `PORTING_LEDGER` `hindsight_bridge` entry moves from `VENDORED` to `RETIRED`.
