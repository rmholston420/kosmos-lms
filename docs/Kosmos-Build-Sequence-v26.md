# Kosmos Build Sequence v26 (kosmos-lms)
...
**Supersedes:** `Kosmos-Build-Sequence-v25.md` (archived at `docs/archive/`). New stages 3.13, 4.7, 5.6, 6.5, 7.4 inserted per ADR-078 to sequence the Tektos-Ultima absorption; existing stages retain their v25 numbering.

**Companion documents (must be present at build time):**
- `Kosmos-Build-Spec-v26.md` — full spec (this sequence is its executable index; §25 covers Tektos absorption)
...
## v26 addendum — Tektos-Ultima absorption stages

_Inserted 2026-09-10 by ADR-078. Full expansion per stage in `docs/plans/KOSMOS_LMS_INTEGRATION_PLAN.md`._

### Stage 3.13 — Immune + Loop-Safety + Thermal port-in

- **Ports touched:** `ImmunePort` (ADR-079), `LoopSafetyPort` (ADR-080), `ThermalPort` (ADR-081), `EventBusPort`, `MemoryPort`, `ResourcePort`
- **What lands:** Tektos-Ultima immune subsystem (12 detectors) at `adapters/immune/tektos/`; loop-safety 3-tier state machine + read-only budget interlock (ADR-088) at `adapters/loop_safety/tektos/`; thermal PID controller (yellow 51 °C / cap 80 °C / red 88 °C, 400 W GPU cap) at `adapters/thermal/tektos/`; kernel boot registry reordered per §25.5.
- **DoD:** contract tests pass for all three ports (Protocol stub + real adapter); kernel boots cleanly with new order; `immune.verdict`, `loop_safety.*`, `thermal.*` envelopes reach `EventBusPort`; every `MemoryPort` write from immune/loop_safety/thermal carries provenance from §25.4 with `confidence=1.0`; simulated-red-state test causes `LLMPort` to refuse inference.

### Stage 4.7 — Sandbox + Planner + Tool Registry port-in

- **Ports touched:** `SandboxPort` (ADR-082), `LLMPort`, `EventBusPort`, `ApprovalPort`, `MemoryPort`
- **What lands:** SandboxPort adapter (Linux namespaces + cgroups) at `adapters/sandbox/tektos/`; Tektos planner at `plugins/tektos/planner/` emitting `tektos.plan.*` on `EventBusPort`; tool registry at `plugins/tektos/tools/` with approval-gated tiers routing through `ApprovalPort`.
- **DoD:** SandboxPort contract test proves resource limits enforced (cpu, memory, wall-time); a scripted plan node round-trips through `EventBusPort`; an approval-required tool call blocks until `ApprovalPort` decision returns; `tektos.tool.*` envelopes carry `provenance=tektos_tool` and `confidence=1.0`.

### Stage 4.8 — Tektos tool-surface reconciliation + filesystem tools

- **Ports touched:** `SandboxPort`, `ApprovalPort`, `MemoryPort`, `EventBusPort`, `ImmunePort` (detector interface), `MCPPort`
- **What lands:** ADR-094 reconciles the two tool-execution surfaces that co-existed after Stage 4.7 (the Stage-3.2 `TektosAgent.call_tool` MCP path and the Stage-4.7 `TektosToolRegistry.invoke` sandbox path) onto the registry. Ships four filesystem tools at `plugins/tektos/tools/filesystem.py` (`file_read`, `file_list` — AUTONOMOUS; `file_write` — HUMAN_REVIEW; `file_delete` — HUMAN_REQUIRED) driven through `SandboxPort.run` with argv-only execution and `network="none"`. Ships `PathTraversalDetector` at `plugins/tektos/tools/detectors/path_traversal.py` implementing the `Detector` protocol from `ports/immune.py`, registered as a pre-approval detector on the registry so `..`, absolute escapes, and symlink escapes never reach `ApprovalPort.propose`. Ships `MCPToolBridge` at `plugins/tektos/mcp/tool_bridge.py` translating `MCPPort.list_tools()` → `ToolDescriptor` registrations with the locked `TEKTOS_TOOL_TIER_MAP` (ADR-037) as tier source and fail-closed `HUMAN_REQUIRED` default. `TektosAgent.call_tool` becomes a thin adapter that delegates to `tool_registry.invoke` when constructed with `tool_registry=...`, preserving the Stage-3.2 inline flow otherwise (Stage 3.2 DoD tests continue to pass unmodified).
- **DoD:** filesystem tools happy-path exercises AUTONOMOUS / HUMAN_REVIEW / HUMAN_REQUIRED approval routing; a `../etc/passwd` invocation publishes `immune.verdict.block` on `EventBusPort`, writes `MemoryPort` with `provenance='immune_verdict'` and `confidence=1.0`, publishes `tektos.tool.denied`, and raises `PathTraversalDetected` — the approval gate is never reached; `MCPToolBridge.discover_and_register` round-trips known + unknown MCP tools with tier resolution honoring the locked map; `TektosAgent.call_tool` returns a `TektosStep` when delegating and continues to raise `RuntimeError` in the legacy path when `mcp`/`apex` are absent.

### Stage 5.6 — Self-improvement + self-repair (gated) — LANDED 2026-09-10 · ADR-095

- **Ports touched:** `SelfModificationPort` (**ADR-090 — PROPOSED; DEFERRED**), `ApprovalPort`, `MemoryPort`, `EventBusPort`
- **What lands:** self-improvement (`plugins/tektos/self_improve/`) and self-repair (`plugins/tektos/self_repair/`) sub-packages **land as propose-only proposers** (ADR-095 D2) with all self-modifying paths gated behind `ApprovalPort` at tier `HUMAN_REQUIRED` (ADR-095 D3); **no** filesystem mutation until ADR-090 ratifies the formal port. Donor engines (`self_repair/engine.py`, `strategies.py`, `workflows.py`, `health_monitor.py`, `effectiveness.py`; `self_improvement/engine.py`; `agents/self_improvement/loop_orchestrator.py`; `self_modification/self_gui_expander.py`, `self_test_expander.py`) are intentionally NOT ported in Stage 5.6 (ADR-095 D1); only the pure-data primitives (`RepairStatus`, `RepairStrategy`, `RepairRecord`, `ExperienceRecord`) are vendored under `adapters/tektos/vendor/`. `apply()` on both proposers raises `NotImplementedError` referencing ADR-090 (belt-and-suspenders on top of the approval-gate deny path).
...
### Stage 7.4 — `search_hybrid` on `DozerDbMemoryAdapter` — **LANDED (2026-09-10 · ADR-099)**

> **STATUS AMENDMENT (2026-09-10, ADR-099):** The original charter ("H1 → H2 hindsight migration") is superseded. H1 (hindsight_bridge adapter) was never built through Stages 3–5 — `plugins/tektos/` consumes `MemoryPort` directly via the DozerDB adapter and no bridge ever existed. Contingency fired: Stage 7.4 is re-scoped to land `MemoryPort.search_hybrid` (ADR-085) on `DozerDbMemoryAdapter` — the substantive work the bridge migration was meant to enable. The `hindsight_bridge` ledger row moves from `PLANNED (Stage 3-5)` to `EVALUATED-REJECTED (Stage 3-5, H1 skipped)`. No repo state changes for the retirement; retire on paper only. See ADR-099 for the six governing decisions.

- **Ports touched:** `MemoryPort.search_hybrid` (ADR-085); `LexicalIndex` (adapter-scoped Protocol, NOT a formal port — symmetric with `GraphBackend` and `TemporalIndex`).
- **What lands:** `LexicalIndex` Protocol + `InMemoryLexicalIndex` BM25-Okapi test backend (k1=1.5, b=0.75) + `RRF_K = 60` constant + `DozerDbMemoryAdapter.search_hybrid` (RRF fusion `score = lw · 1/(60+rank_lex) + sw · 1/(60+rank_sem)` per ADR-085 verbatim, `validate_hybrid_weights` guard first, `NotImplementedError` when `LexicalIndex` unwired — honesty rule, semantic-payload-wins on id collision, `min_score` filter on fused score) + `write_event` lexical mirror side-effect + plugin `FakeMemoryPort` conformance updates (Tektos test_openspec / test_repomap / test_tektos_agent + Zetesis `ZetesisMemoryStub` + `test_real_adapter_factory` `_FakeMemoryPort`) + 11 new contract tests (`adapters/memory/dozerdb/test_search_hybrid_contract.py`: LexicalIndex Protocol conformance, weight-guard boundary + rejection, honesty rule, write_event mirror, lexical-only fusion, both-legs fusion math, payload preference, min_score filter, corpus filter, limit truncation).
- **DoD:** all 11 new contract tests green; the 6 pre-existing `MemoryPort` protocol-drift failures resolve automatically (adapter + 5 fakes); full regression = **1437 passed / 0 failed / 14 skipped** — the first Kosmos stage to reach zero pre-existing failures.
- **Explicit exclusions (deferred to Stage 7.4+1):** real `DozerDbLexicalIndex` backed by DozerDB/Neo4j Lucene fulltext (`CALL db.index.fulltext.queryNodes(...)`); wiring the lexical lane at kernel boot for the ZetesisPlugin factory (currently opt-in via `lexical=` kwarg).
- **ADRs:** ADR-085 (MemoryPort.search_hybrid surface + RRF formula — pre-existing), **ADR-099** (Stage 7.4 re-scope: H1 skipped, `search_hybrid` lands, `LexicalIndex` adapter-scoped, `InMemoryLexicalIndex` ships now, `DozerDbLexicalIndex` deferred, no `ports/memory.py` changes).

### Stage 7.4+1 — real `DozerDbLexicalIndex` (Neo4j Lucene fulltext) — **LANDED (2026-09-10 · ADR-100)**

- **Ports touched:** none new. Implements the `LexicalIndex` Protocol locked by ADR-099 D3 (adapter-scoped Protocol in `adapters/memory/dozerdb/adapter.py`; not a formal `ports/` port).
- **What lands:** `adapters/memory/dozerdb/dozerdb_lexical_index.py` — production `LexicalIndex` backed by DozerDB's Lucene fulltext index. Mirrors `DozerDbGraphBackend`'s shape (lazy `neo4j.AsyncGraphDatabase` import; async driver + per-call `AsyncSession`; `_init_error` capture; sync + non-throwing `is_healthy`; idempotent async `close` that swallows driver errors into a warning log). `index_event` runs `MERGE (n:MemoryEvent {id: $id}) SET n.text = $text, n.as_of = datetime($as_of_iso), n.corpus_name = $corpus`, where `text` is the shared `_lex_text_from_payload` concatenation (extracted from `adapter.py` so both backends tokenise identical text). `search_lexical` runs `CALL db.index.fulltext.queryNodes($index_name, $query) YIELD node, score` with a post-YIELD `WHERE $corpus IS NULL OR node.corpus_name = $corpus` filter, ordering, and limit; rehydrates a minimal `MemoryHit.payload = {"text": ..., "attributes": {"corpus_name": ...}}` (full-triple rehydration is `MemoryPort.query_temporal`'s job). Bootstraps the fulltext index lazily on first `index_event`/`search_lexical` via `CREATE FULLTEXT INDEX $index_name IF NOT EXISTS FOR (n:MemoryEvent) ON EACH [n.text]`, gated by `self._index_ready`. Both `label` and `index_name` are constructor-tunable, both validated by the `DozerDbGraphBackend` identifier guard (`^[A-Za-z_][A-Za-z0-9_]*$`) before literal Cypher interpolation — required because `CREATE FULLTEXT INDEX` does not accept `$name` parameter substitution. Lucene procedure failures degrade to `[]` with a warning log (symmetric with `DozerDbMemoryAdapter.write_event`'s log-only lexical-mirror discipline). Exported from `adapters/memory/dozerdb/__init__.py`. Contract tests: 25 fast-tier tests (mocked driver) + 1 env-gated live-tier test (`KOSMOS_STAGE_74_REAL_DOZERDB_LEXICAL=1`) in `adapters/memory/dozerdb/test_dozerdb_lexical_index_contract.py`.
- **DoD:** all 25 fast-tier contract tests green; Protocol conformance via `isinstance(..., LexicalIndex)`; identifier guard rejects Cypher-injection shapes on both `label` and `index_name`; bootstrap idempotency across repeated `index_event` calls (1 CREATE FULLTEXT INDEX + N MERGEs); Cypher-shape assertions on both bootstrap and query paths; corpus filter propagation; minimal-payload rehydration; procedure-failure degradation to `[]` + warning log; `close` idempotency + driver-error swallowing; `is_healthy` false after close and after driver-init failure; configured `database` name plumbed through `session()`. Full regression: **1462 passed / 0 failed / 15 skipped** (baseline 1437/0/14 → +25 new pass, +1 new live-tier skip, zero new fail).
- **Explicit exclusions (deferred to Stage 7.4+2):** kernel-boot wiring of `DozerDbLexicalIndex` into the `ZetesisPlugin` factory so `MemoryPort.search_hybrid` no longer raises `NotImplementedError` in production (adds Docker compose dependency to CI critical path; deserves a dedicated wiring ADR). Lucene reserved-character escaping (a plugin concern; the adapter passes queries through unchanged so callers can use Lucene syntax intentionally).
- **ADRs:** ADR-099 (Stage 7.4 re-scope; `LexicalIndex` Protocol locked — pre-existing), **ADR-100** (Stage 7.4+1 real `DozerDbLexicalIndex`: adapter shape mirrors `DozerDbGraphBackend`; index name / label / bootstrap Cypher; post-YIELD corpus filter; minimal-payload rehydration; log-only degradation on Lucene failure; boot-time wiring deferred to Stage 7.4+2).

### Stage 7.4+2 — kernel-boot wiring of `DozerDbLexicalIndex` — **LANDED (2026-09-10 · ADR-101)**

- **Ports touched:** none new. Extends `kernel/app.py::_boot_memory` (the single canonical construction site for `DozerDbMemoryAdapter` per ADR-063 shared-MemoryPort principle) with an opt-in lexical-lane wiring block. The `plugins/zetesis/adapters/real/factory.py::build_stage_6_5_zetesis_plugin` factory signature is unchanged — Zetesis inherits the wired lexical lane transparently through the existing `memory=registry.memory` seam.
- **What lands:** one new env var `KOSMOS_MEMORY_LEXICAL` with two allowed values (`off` — default; `dozerdb` — opt-in production wiring). When `dozerdb`, `_boot_memory` constructs `DozerDbLexicalIndex` sharing the same `KOSMOS_DOZERDB_URI` / `_USER` / `_PASSWORD` / `_DATABASE` env vars as the graph backend (ADR-008 single-canonical-store rule; one Bolt endpoint per memory subsystem). A reject-shape guard raises `RuntimeError` if `KOSMOS_MEMORY_LEXICAL=dozerdb` is set with `KOSMOS_MEMORY_BACKEND` not `dozerdb`; another guard raises on unknown lexical values, enumerating the allowed set. Boot-time `is_healthy()` check discards the constructed index on failure (fail-closed to `lexical=None` with a warning log citing ADR-101 D3; `search_hybrid` reverts to `NotImplementedError` per ADR-085). Construction failures inside the wiring helper are swallowed into a warning log via the same D3 path (also cited in the log). Successful wiring emits an INFO log `kosmos.memory.lexical: wired (ADR-101); backend=dozerdb uri=... database=...`. All logging routes through `logging.getLogger(__name__)` matching the existing `kernel/app.py` convention. Six fast-tier tests at `tests/kernel/test_stage_7_4_2_lexical_wiring.py` cover: unset → lexical=None + search_hybrid raises; explicit `off` → same; `dozerdb` + in-memory backend → reject-shape error in `registry.errors["memory"]`; `dozerdb` + dozerdb backend + healthy driver → real `DozerDbLexicalIndex` wired + `is_healthy()` True; `dozerdb` + monkeypatched failing driver → lexical falls through to None + ADR-101 D3 warning captured; unknown value `mystery` → reject error enumerating allowed values.
- **DoD:** all 6 acceptance tests green; baseline regression **1462 passed / 0 failed / 15 skipped** PRESERVED (`tests/kernel/` is a pre-existing orphan directory outside `pyproject.toml` `testpaths` — noted in DEBUG_LOG for future cleanup; new tests run via explicit invocation, matching the pattern for the sibling `test_stage_6_5_7_gnosis_retrieval.py` and other `tests/kernel/test_stage_6_5_*` files). `plugins/zetesis/adapters/real/factory.py` unchanged (no signature growth; factory inherits the wired lane through the existing `memory=registry.memory` seam). No changes to `LexicalIndex` Protocol, `DozerDbLexicalIndex` constructor, `search_hybrid` fusion math, or `PORTING_LEDGER.md` (wiring-only slice; no new vendored components).
- **Explicit exclusions (deferred to future stages):** Lucene reserved-character escaping in `search_lexical` (ADR-100 deferral holds; TBD Stage 7.4+3 if needed). Adding `tests/` to `pyproject.toml` `testpaths` (would surface 13 pre-existing failures in the orphan `tests/` tree — separate triage + ADR beyond this slice). Live-tier kernel-boot integration test (would duplicate coverage vs the existing `KOSMOS_STAGE_74_REAL_DOZERDB_LEXICAL=1` live-tier test at `adapters/memory/dozerdb/test_dozerdb_lexical_index_contract.py`).
- **ADRs:** ADR-100 (production `DozerDbLexicalIndex` — pre-existing; Stage 7.4+2 discharges its D6 deferral pointer), **ADR-101** (Stage 7.4+2 kernel-boot lexical wiring: env schema `KOSMOS_MEMORY_LEXICAL={off,dozerdb}`; wiring in `_boot_memory` not the factory; opt-in not automatic; shared Bolt endpoint; boot-time `is_healthy` check with fail-closed fall-through; construction failure caught by outer `_try("memory")` — no separate subsystem key; two-tier tests fast + existing live).


### Stage 8.0 — `RelationalMemoryPort` — Postgres 5th memory layer — **LANDED (2026-09-10 · ADR-102)**

- **Ports touched:** **new formal port** `RelationalMemoryPort` at `ports/relational_memory.py` — the 23rd. Combines an R1 audit ledger (`record_event` / `query_events`) and an R2 episodic-narrative store (`write_narrative` / `search_narratives`). Both `LedgerRow` and `NarrativeHit` return-types are frozen slotted dataclasses. `transaction()` is an async context manager exposing a `_Tx` handle with the same `record_event` + `write_narrative` shape for atomic multi-write flows. `is_healthy() -> bool` is sync + non-throwing; `close()` is async + idempotent. Zero-trust helpers `validate_confidence` + `validate_provenance` are exported alongside the Protocol for adapter reuse and downstream call-site enforcement.
- **What lands:**
  - `NoOpRelationalMemoryAdapter` (`adapters/relational_memory/noop/adapter.py`) — aiosqlite 0.22 backed, `:memory:` by default. Real BEGIN/COMMIT/ROLLBACK with autocommit mode (`isolation_level=None`) + a `_tx_depth` guard so per-write commits are only issued outside an explicit `transaction()`. Every write path calls `validate_confidence` + `validate_provenance` at the port boundary. `search_narratives` honestly degrades when an `embedding` is passed (sqlite has no vector type) — the embedding is ignored and rank falls back to substring `LIKE` over `body`+`title`. Contract test file: 23 fast tests at `adapters/relational_memory/noop/test_contract.py`.
  - `PostgresRelationalMemoryAdapter` (`adapters/relational_memory/postgres/adapter.py`) — asyncpg 0.31 + `pgvector.asyncpg.register_vector`; lazy pool creation via `asyncpg.create_pool(min_size=2, max_size=10, init=...)`; sync non-throwing `is_healthy()` returns True on construction and defers connection validation to first async call (ADR-100 D1 pattern). Pure-tsvector branch when `embedding=None` (`ts_rank_cd(body_tsv, plainto_tsquery('english', $q), 32)`), RRF-fused hybrid branch when `embedding` is provided (k=60 over ts_rank and pgvector cosine `<=>`, joined `FULL OUTER JOIN` on `narrative_id`). Contract test file: 3 import-tier tests + 5 live-tier tests at `adapters/relational_memory/postgres/test_contract.py` gated on `KOSMOS_STAGE_80_REAL_POSTGRES=1` + `KOSMOS_POSTGRES_URI`.
  - Alembic scaffold at `adapters/relational_memory/postgres/migrations/` — `alembic.ini` (DSN read from env at runtime via `env.py`; normalises `postgres://` → `postgresql+asyncpg://`), `env.py` (async + offline modes; NullPool), `script.py.mako`, `versions/001_initial.py` (creates `ledger_events` + `narratives` with tsvector generated column, GIN body/tsv + tags indexes, HNSW pgvector index on `vector(1536)`; installs `pg_trgm`, `pgcrypto`, `vector` extensions unconditionally and `pg_uuidv7` tolerantly through a DO block; authors a `kosmos_uuid7()` PL/pgSQL wrapper that dispatches to `uuid_generate_v7()` when installed, else `gen_random_uuid()`).
  - Kernel wiring at `kernel/app.py::_boot_relational_memory` — env-gate `KOSMOS_RELATIONAL_MEMORY={off,noop,postgres}` (default `off` silent); `postgres` requires `KOSMOS_POSTGRES_URI`; unhealthy adapter → `registry.relational_memory=None` with warning log citing ADR-102 D6; unknown value → `RuntimeError` captured in `registry.errors["relational_memory"]` enumerating the allowed set. New `_BootRegistry.relational_memory: Any = None` slot with an inline docstring pointing downstream call sites at the health-check pattern.
  - Kernel wiring tests at `tests/kernel/test_stage_8_0_relational_memory_wiring.py` — 6 fast acceptance tests covering unset / off / noop / postgres-without-DSN / postgres-with-unreachable-DSN (constructs but pool would fail lazily) / unknown-value.
- **DoD:** all 34 new tests green (23 noop + 3 postgres import-tier + 6 kernel-wiring in Cloud; 5 postgres live-tier skipped without `KOSMOS_STAGE_80_REAL_POSTGRES=1`). Full regression **1488 passed / 0 failed / 20 skipped** in Cloud (baseline 1462/0/15 → +26 new pass, +5 new live-tier skip, zero new fail). Baseline preserved.
- **Explicit exclusions (deferred to future stages):**
  - R3 (pgvector as an alternative `VectorPort` backend) — explicit ADR-102 deferral; Qdrant remains the sole primary `VectorPort` at Stage 8.0. A future ADR opens the R3 surface if operational needs warrant collapsing the two vector backends.
  - Downstream call-site adoption — `record_event` migrations for approvals/tool-invocation/budget/immune subsystems, and `write_narrative` adoption inside the Tektos reflection loop, ship in Stages 8.1–8.7 as those runtime slices land per Plan v2.
  - Colossus live-tier verification — the 5 skipped postgres live-tier tests exercise on Colossus after the user runs `alembic upgrade head` against a real Postgres 18 + pgvector + pg_uuidv7 instance; part of the Stage 12 deployment slice.
- **ADRs:** **ADR-102** (Stage 8.0 RelationalMemoryPort as 5th memory layer: 23rd formal port; R1 audit ledger + R2 episodic-narrative store; two adapters noop + postgres; RRF k=60 hybrid; zero-trust write contract parity with ADR-008; kernel env-gate with ADR-101-style degrade; Alembic migrations; no auto-migrate on boot per ADR-063; R3 deferred). Preserves and reinforces ADR-007 (adapters are new subpackages under `adapters/relational_memory/`; no cross-plugin imports); ADR-008 (zero-trust semantics mirrored at the port layer); ADR-063 (no implicit schema changes on kernel boot); ADR-101 (identical degrade pattern for the health-check fall-through).
