...
|---|---|---|
| **P — Preserved** | **129** (audit baseline 35 + T-bucket 47 reconciled/promoted + Stage 13.1–13.14 ports — reconciled live against the route table, 2026-09-26) | kernel/plugin referent exists and is live |
...
| **T — To-port** | **2** (`POST /api/mcp/connect` + `GET /api/mcp/status` — the ONLY real unported surface remaining; donor `MCPClient` at `tools/registry.py:319` never ported, kernel vendored `tool_registry_donor.py` is a partial vendor that omits it) | no referent yet; each becomes a work item before deletion |

Count reconciliation note (2026-09-26): the 13.13 header reported "125 P / 25 D / 2 T = 152" — that total was a miscount (the route table has **154** rows). Re-derived directly from the table: **129 P / 23 D / 2 T = 154**. Also corrected on this pass: the session claim that "mcp ×2 was DONE / committed before 13.8" is **false** — no MCP route code, test, or commit exists; the 2 MCP rows remain the only T-bucket work items.
