...
### Disposition counts (152 HTTP + 2 WS = 154)
...
| `POST /api/mcp/connect` | **P (Stage 13.15, 2026-09-26)** — donor `MCPClient` (`tools/registry.py:319-543`, ~225 LOC, pure generic substrate: HTTP `tools/list` via stdlib urllib, SSE via lazy aiohttp, per-tool `tools/call` handler closures) → `kernel/mcp_client.py` byte-verbatim (differences: `ToolDefinition` import from `kernel.tool_registry`, logger name). Route wire-verbatim (donor main.py:2948-2954): ok `{status, url, tools_imported, tool_names}` / error `{status: "error", url, error, tools_imported: 0}`. Boot (donor main.py:287-303): `MCPClient(registry=_tool_registry)` + best-effort `connect()` to `KOSMOS_MCP_SERVER_URL` (donor `TEKTOS_MCP_SERVER_URL`, default `http://127.0.0.1:3001/mcp`) / `KOSMOS_MCP_TRANSPORT`; failure non-fatal. Live-tested 4/4 (`tests/kernel/test_adr141_s1315_mcp_routes.py`: real substrate against a real local HTTP MCP server — tools/list import + tools/call round-trip through `/api/tools/{name}/execute`, no mocks) |
| `GET /api/mcp/status` | **P (Stage 13.15, 2026-09-26)** — donor route wire-verbatim (main.py:2930-2939): `{connected, url, imported_count}` reading `_server_url`/`_imported_count`; uninitialized → `{connected: false, url: null}`. Same substrate/boot as `POST /api/mcp/connect` above |
...
| `GET /api/rag/status` | **P (Stage 13.14, 2026-09-26)** — kernel-native `GET /api/rag/status` (ADR-124 D1, Stage 11.8 — predates this audit; live-reconciled here). Donor wire `{status, stats}` is a SUBSET of the kernel wire (additive `healthy`, `embedder`, `vector` blocks over the ADR-124 D1 embeddings lane :8091 + vector store; `stats` retained). Donor `not_initialized` gate preserved. Live-verified :8000 → 200 `{status:"initialized", healthy:true, embedder:{available, model:qwen3-embedding-0.6b, :8091}, vector:{available}, stats...}`. Test `tests/kernel/test_stage_11_8_adr_124_rag_status.py`. (Distinct from Stage 13.10's `/api/ragRetriever/status` — the donor's OWN vector-index substrate; both live.) |
| `GET /api/ragRetriever/status` | **P (Stage 13.10, 2026-09-26)** — donor `tektos/runtime/rag_retriever.py` (858 LOC, 100% stdlib + aiosqlite) → `kernel/rag_retriever.py` byte-verbatim (generic vector-index substrate → kernel per layering rule); `registry.tektos_rag_retriever` booted with `project_root` = Kosmos repo root + db at `data/tektos_rag.db` (gitignored `*.db`), the donor's `EmbedderClient` seam (`embed_batch`/`embed` → `.embeddings`) bridged to the kernel-owned ADR-124 D1 lane via a tiny `_EmbedderBridge` in the composition root (ADR-007, substrate untouched); async `start()` (SQLite open) scheduled fire-and-forget as in the donor boot (main.py:1464-1470); donor route wire verbatim (`initialized` + db_path + initialized flag, `not_initialized` gate-off at 200). Live-verified: `initialized:true`, db path correct, embedder lane :8091 healthy; real keyword-retrieval unit test (no mocks, temp tree). 3 tests. |
...
| `GET /api/voice/state` | **P (Stage 13.12, 2026-09-26)** — donor `tektos/voice.py` (310 LOC, all third-party deps lazy: edge-tts / faster-whisper / pydub / numpy; zero `tektos.*` imports — "tektos" strings are logger name, wake word, error messages, kept verbatim) → `kernel/voice.py` BYTE-VERBATIM (generic STT/TTS/VAD/wake-word orchestration → kernel per the layering rule). `registry.tektos_voice` booted at the composition root with the donor's non-fatal `await initialize()` semantics (main.py:899-910: failure → slot None → routes in their "not initialized" state; Whisper large-v3-turbo on CPU per donor defaults) + kernel-side `KOSMOS_TEKTOS_VOICE` gate (default on). Donor wire verbatim: `state` → 200 `{"error": "Voice system not initialized"}` when slot None (NOT 503 — donor-verbatim divergence from the vision 503 pair), `stt`/`tts` → 503 pair. 4 tests: REAL Whisper (faster-whisper-base, cached; donor's own `TEKTOS_WHISPER_MODEL` env knob) transcribing a real generated silence WAV through pydub → faster-whisper, REAL edge-tts (Microsoft neural voice, real network) synthesizing a real MP3 (ID3/magic-byte verified, donor's `attachment; filename=tektos_speech.mp3` header), state envelope 5-field match, gate-off surfaces — no mocks. |
