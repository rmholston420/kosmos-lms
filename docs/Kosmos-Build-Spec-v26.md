...
| `RelationalMemoryPort` | `NoOpRelationalMemoryAdapter` (aiosqlite in-memory, CI + dev — MIT) → `PostgresRelationalMemoryAdapter` (asyncpg 0.31 + pgvector 0.5 + pg_uuidv7, production — Apache-2.0 / MIT / MPL-2.0). Kernel wiring gated by `KOSMOS_RELATIONAL_MEMORY={off,noop,postgres}`; Postgres branch requires `KOSMOS_POSTGRES_URI`. Schema managed by Alembic (`adapters/relational_memory/postgres/migrations/`); no auto-migrate on boot (ADR-063 + ADR-102 D7). | `async record_event(*, kind, session_id, agent_id, payload, confidence, provenance) -> str` (R1 audit ledger), `async query_events(*, kind_prefix=None, session_id=None, agent_id=None, since=None, until=None, limit=100) -> tuple[LedgerRow, ...]`, `async write_narrative(*, session_id, agent_id, title, body, tags, embedding, confidence, provenance) -> str` (R2 episodic-narrative store), `async search_narratives(*, query, embedding=None, session_id=None, agent_id=None, tags=(), limit=20) -> tuple[NarrativeHit, ...]` (pure tsvector when embedding=None; RRF k=60 fusion of ts_rank_cd + pgvector cosine when embedding provided), `transaction()` async context manager, `is_healthy() -> bool` (sync, non-throwing), `async close()` (idempotent). Zero-trust write contract (ADR-102 D4): every `record_event` and `write_narrative` validates `confidence ∈ [0, 1]` and non-empty `provenance` at the port layer. — see **ADR-102** |
...
| ADR-102 | Stage 8.0 · RelationalMemoryPort as the 5th memory layer (Postgres 18 + pgvector + pg_uuidv7). Introduces the 23rd formal port `RelationalMemoryPort` (`ports/relational_memory.py`) that combines an R1 audit ledger (sessions, approvals, tool-invocation, ADR-088 budget history, ADR-092 immune events) and an R2 episodic-narrative store with tsvector + hnsw pgvector hybrid search (RRF k=60 fusion of `ts_rank_cd` and pgvector cosine). Ships two adapters at Stage 8.0: `NoOpRelationalMemoryAdapter` (aiosqlite in-memory, required for CI + dev) and `PostgresRelationalMemoryAdapter` (asyncpg 0.31 + pgvector 0.5). Schema owned by Alembic (`adapters/relational_memory/postgres/migrations/`); primary keys default via a `kosmos_uuid7()` PL/pgSQL wrapper that dispatches to `uuid_generate_v7()` (pg_uuidv7, MPL-2.0) when installed, else falls back to `gen_random_uuid()`. Zero-trust write contract (D4): every `record_event` and `write_narrative` validates `confidence ∈ [0, 1]` and non-empty `provenance` at the port layer — mirrors ADR-008 MemoryPort discipline. Kernel wiring via `KOSMOS_RELATIONAL_MEMORY={off,noop,postgres}` env-gate (default `off`); postgres branch requires `KOSMOS_POSTGRES_URI`. Health failure follows the ADR-101 degrade pattern — unhealthy adapter leaves `registry.relational_memory=None` with a warning log; downstream call sites tolerate `None`. Kernel does NOT auto-migrate on boot (ADR-063 no-implicit-schema-changes rule; operators run `alembic upgrade head` explicitly). R3 (pgvector as a `VectorPort` alternative) explicitly rejected at Stage 8.0 and deferred to a future ADR to preserve Qdrant as the primary `VectorPort` backend and to avoid conflating episodic-narrative retrieval with generic vector KV. Rejects: extending `MemoryPort` (violates single-responsibility — the graph memory adapter's zero-trust surface and the SQL adapter's ledger + narrative surfaces have different write shapes); SQLAlchemy Core (asyncpg + raw SQL keeps the adapter small and preserves Postgres-native features `tsvector`/`pgvector`/`pg_uuidv7`). Live-tier tests gated by `KOSMOS_STAGE_80_REAL_POSTGRES=1` + `KOSMOS_POSTGRES_URI`; fast-tier tests always run against the noop adapter. Contract-test count: 23 noop fast + 3 postgres import-tier + 5 postgres live-tier (skipped in Cloud) + 6 kernel-wiring fast = **34 new tests**, all green in Cloud with the baseline preserved (1462 → 1488 passed, 15 → 20 skipped, 0 failed). PORTING_LEDGER additions (five entries): aiosqlite 0.22.1 (MIT), asyncpg 0.31.0 (Apache-2.0), pgvector-python 0.5.0 (MIT), pg_uuidv7 (MPL-2.0, optional server-side), Alembic 1.19.2 (MIT, migrations-only). | **Ratified v25** | Stage 8.0 |
...
**Baseline for future revisions:** any addendum, agentic scan, or new ADR authored after v26 lives alongside v26 (not replacing it) until formally folded into v27.


---

## 25. Tektos absorption

_Appended 2026-09-10 by ADR-078 as part of the v26 cut. Codifies the kosmos-lms integration ratified in ADR-077 and the twelve downstream ADRs (079–090)._

### 25.1 Absorption principle

kosmos-lms is the merged successor of `rmholston420/kosmos` and `rmholston420/tektos-ultima`. Tektos was always intended to be the coding-agent plugin for Kosmos; ADR-077 executes that intent by folding the entire Tektos-Ultima runtime into the `plugins/tektos/` scaffold under Kosmos discipline. The Kosmos plugin scaffold is preserved, not replaced; the ten ratifying ADRs (ADR-036/037/038/041/042/044/045/046/063/065) remain load-bearing and their DoDs stand. Tektos-Ultima subsystems land under `plugins/tektos/` and `adapters/<port>/tektos/` behind formal ports. All ports keep the zero-trust write guarantee (§7) non-bypassable; all cross-plugin coupling is events-only (ADR-007).

### 25.2 Frontend strategy

The Kosmos Next.js shell (`ui/`, Next 16.2.11) hosts the Tektos Next.js UI (`plugins/tektos/frontend/`, Next 15.4, 40 panels) as an **iframe/microfrontend**, mounted by the kernel under `/tektos/frontend` on the same origin via reverse proxy. This is codified by ADR-089, which adds `PanelKind.IFRAME` to `FrontendContractPort`. Rejects: a unified Next.js shell (forces Next 15→16 reconciliation and Monaco + xterm + D3 dependency merge on the critical path); a separate-origin frame (breaks cookie/session sharing and forces CORS work).
...
Every `MemoryPort` write originating from a Tektos-side subsystem MUST carry one of these thirteen `provenance` values. `confidence` is always required per §7.

| Provenance | Emitter | Typical `confidence` |
|---|---|---|
| `tektos_agent` | Iteration-runtime turn output | 0.7–0.9 |
| `tektos_reflection` | Planner reflection step | 0.5–0.8 |
| `tektos_planner` | Plan graph node | 0.6–0.9 |
| `tektos_self_modification` | Self-improvement / self-repair path | ≤ 0.9 (never 1.0; gated behind ADR-090) |
| `immune_system` | Immune scan raw signal | 1.0 |
| `immune_verdict` | Immune verdict decision | 1.0 |
| `loop_safety` | Loop-safety state transition | 1.0 |
| `thermal` | Thermal sample or pressure event | 1.0 |
| `sandbox` | SandboxPort result envelope | 1.0 |
| `hindsight_bridge` | Bridge-adapter read/write (Stages 3–5 only) | passthrough |
| `tektos_tool` | Tool-registry invocation record | 1.0 (invocation), 0.5–1.0 (result) |
| `tektos_gateway` | Gateway-proxy envelope replay | 1.0 |
| `tektos_frontend` | User-initiated action from Tektos UI | 1.0 |

The port-level guard in `ports/memory.py` (`validate_zero_trust_write`) rejects writes missing either field. AMG (Agent Memory Guard v0.2.2) runs as a second policy layer atop the guard, never as a replacement.

### 25.5 Kernel boot order after Stage 3.13

After Tektos runtime port-in, the kernel boot registry order becomes:

```
immune → loop_safety → thermal → notification → frontend_contract → resource →
event_bus → approval → sandbox → mcp → memory → llm →
phrouros → praxis → zetesis → tektos
```

Rationale: safety-authoritative ports (immune, loop_safety, thermal) boot first so downstream ports can subscribe to their events and refuse to serve during a red state. `sandbox` sits after `approval` so gated writes route through both. `tektos` boots last because it consumes every prior port.

### 25.6 Loop-safety read-only budget interlock

Per ADR-088, `LoopSafetyPort` maintains a per-turn read-only tool-call budget (default: 10 calls). When the budget is exhausted, the port publishes `loop_safety.read_only_budget_exhausted` on `EventBusPort` and the Tektos runtime is forced to complete the turn with text only (no further tool calls). This is a hard interlock — the runtime cannot bypass it. The budget resets at turn boundaries. Rationale: prevents pathological read-only loops (grep→grep→grep) that consume tokens without producing agent progress, per the read-only-tool-budget concept in project knowledge.

### 25.7 Reserved ports and configuration

Tektos ports reserved on Colossus loopback:

| Port | Service | Owning subsystem |
|---|---|---|
...
|---|---|---|
| Yellow | 51 °C | `thermal.pressure` event; ResourcePort back-pressure begins |
| Cap | 80 °C | 400 W GPU power cap engaged |
| Red | 88 °C | Emergency throttle; `thermal.red` event; LLMPort refuses new inference |

Loop-safety caps (default per turn):

| Cap | Value |
|---|---|
| `max_turns` | 15 |
| `max_tokens_total` | 65 536 |
| `max_wall_time_seconds` | 300 |
| `repetition_window` | 3 |
| Read-only tool budget (§25.6) | 10 |
